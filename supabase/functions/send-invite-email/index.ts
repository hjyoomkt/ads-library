...
interface InviteEmailRequest {
  inviteCode: string;
  invitedEmail: string;
  inviteType: 'new_organization' | 'new_brand' | 'existing_member' | 'new_agency';
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 회원가입 링크 생성
    const appUrl = Deno.env.get('APP_URL') || 'https://library.zestdot.com';
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #ffffff;">
  <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto;">
    ...
    <tr>
      <td style="padding: 60px 40px;">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="margin: 0 0 40px;">
          <tr>
            <td align="center">
              <a href="${signupUrl}" target="_blank" style="display: inline-block; padding: 18px 60px; font-size: 16px; font-weight: 600; color: #ffffff; background-color: #2d3748; text-decoration: none; border-radius: 50px; text-align: center;">
                회원가입 하기
              </a>
            </td>
          </tr>
    ...
    const resendResponse = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: '제스트러리 <invite@library.zestdot.com>',
        to: invitedEmail,
        subject: '제스트러리 초대장',
        html: emailHtml,
      }),
