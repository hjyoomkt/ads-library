...
function InviteSignUpForm({ initialCode, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 세션이 없으면 에러 (이메일 확인이 필요한 경우)
      if (!authData.session) {
        throw new Error('이메일 확인이 필요합니다. Supabase 설정에서 "Enable email confirmations"를 비활성화해주세요.');
      }

      // 세션을 명시적으로 재설정 (Supabase 클라이언트 동기화)
      console.log('🔐 세션 재설정 시작');
      const { data: { session: currentSession } } = await supabase.auth.getSession();
      console.log('🔐 현재 세션 상태:', {
        hasSession: !!currentSession,
        userId: currentSession?.user?.id,
        accessToken: currentSession?.access_token ? '토큰 있음' : '❌ 토큰 없음',
      });
      ...
      console.log('📝 Users 테이블 insert 시도:', {
        id: authData.user.id,
      ...
      console.log('✅ Users 테이블 insert 성공');
