...
export default function APIStatus(props) {
  ...
  const getStatusDisplay = (status) => {
    switch (status) {
      case 'success':
        return { icon: MdCheckCircle, color: 'green.500', label: '정상', badgeColor: 'green' };
      case 'error':
        return { icon: MdOutlineError, color: 'red.500', label: '오류', badgeColor: 'red' };
      case 'pending':
        return { icon: MdSchedule, color: 'orange.500', label: '대기', badgeColor: 'orange' };
      default:
        return { icon: MdSchedule, color: 'gray.500', label: '알 수 없음', badgeColor: 'gray' };
    }
  };
