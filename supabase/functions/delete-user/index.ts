...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1. 사용자 정보 조회
    // ads-library: users.advertiser_id 직접 참조 (user_advertisers 테이블 없음)
    ...
    if (isAdvertiserAdmin && !is_brand_deletion && !is_agency_deletion) {
      ...
      // ads-library: users 테이블에서 직접 확인 (user_advertisers 테이블 없음)
